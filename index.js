...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // A more robust way to initialize user progress to prevent errors
    function getInitialProgress() {
        const defaultProgress = { days: [], darkness: 0.8 };
        try {
            const storedProgress = JSON.parse(localStorage.getItem('healingProgress'));
            // Merge stored data with defaults to ensure all keys exist
            return { ...defaultProgress, ...storedProgress };
        } catch (e) {
            // If parsing fails, return defaults
            return defaultProgress;
        }
    }
    let userProgress = getInitialProgress();
